...
DrawFormattedText(window,breakText,sx,sy,fontColor);
Screen('Flip',window);
...
elseif showWarning
    ...
    AuditiveFeedback(1); WaitSecs(.3);
    AuditiveFeedback(1); WaitSecs(.3);
    AuditiveFeedback(1); WaitSecs(.3);
    
    while GetSecs - t0 < breakDuration
        remainingTime_inSecs = breakDuration - ceil(GetSecs - t0) + 1;
        [nx ny] = DrawFormattedText(window,breakText,sx,sy,fontColor);
        DrawFormattedText(window,['\n\nexperiment resumes in\n' num2str(remainingTime_inSecs) ' seconds'],sx,ny,fontColor);
        Screen('Flip',window);
